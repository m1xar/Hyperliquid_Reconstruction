...
func AttachAuth(client *resty.Client, creds Credentials) {
	client.SetPreRequestHook(func(_ *resty.Client, req *http.Request) error {
		ts := fmt.Sprint(time.Now().UTC().UnixMilli())
		nonce := uuid.NewString()
		method := req.Method
		path := req.URL.RequestURI()

		sign := signRequest(creds.Secret, path, method, ts, nonce, "")
...
func signRequest(secret, path, method, timestamp, nonce, body string) string {
...
func DoGetRaw(client *resty.Client, baseURL, path string, params map[string]string) (string, error) {
	req := client.R()
	for k, v := range params {
		req.SetQueryParam(k, v)
	}
